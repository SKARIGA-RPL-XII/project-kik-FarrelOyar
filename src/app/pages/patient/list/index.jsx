  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (res.success) {
        ...
        setPagination(res.data.pagination); // ambil dari API
      }
      setTimeout(() => {
        setLoading(false);
      }, 500);
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRow: (row) => {
        skipAutoResetPageIndex();
        setPatient((old) =>
          old.filter((oldRow) => oldRow.patients_id !== row.original.patients_id),
        );
      },
      deleteRows: (rows) => {
        skipAutoResetPageIndex();
        const rowIds = rows.map((row) => row.original.patients_id);
        setPatient((old) => old.filter((row) => !rowIds.includes(row.patients_id)));
      },
      setTableSettings,
      setViewType,
    },
        ...
        >
          ...
          >
            ...
            >
              ...
              {viewType === "list" && (
                <ListView table={table} flexRender={flexRender} rows={rows} loading={loading}/>
              )}
